...
func (s *Service) getRankFromVLR(comment *reddit.Comment) (*RankResponse, error) {
	...
	if resp.StatusCode != 200 {
		return nil, fmt.Errorf("request returned non-200: %d", resp.StatusCode)
	}
...
func (s *Service) RankComment(comment *reddit.Comment) error {
	...
	rankResp, err := s.getRankFromVLR(comment)
	if err != nil {
		return err
	}
